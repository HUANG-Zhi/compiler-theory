/*  
	*For java src scanner

	*use scanner_start do init work and scanner_end do end work

	*between scanner_start and scanner_end ,use scanner_get_unin
	to get a sacnner word from src,return true means success ,or 
	the src file is end（current_unit.code = 0x130）;
	
	* every time after you using scanner_get_unin,the new scanner
	word is stored in the gloable struct Unit variable current_unit,
	you can use extern struct Unit current_unit in your src file to
	use it;
*/
#include <stdio.h>
#include <string.h>
...
/*output unit*/
struct Unit{
	char value[UNITVALUESIZE];
	int code;
	char type[UNITVALUESIZE];
	int valuei;
	bool conmplete;
	bool error;
	long int line;
	char erinfo[UNITVALUESIZE];
};
/*functions*/
